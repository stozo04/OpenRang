...
2. **Every file in `docs/lessons_learned/`** — start with `README.md`, then read 001 through 011 in order. **Non-negotiable** per CLAUDE.md "Required Reading." Lesson 003 (DataStore IOException wrapping), Lesson 004 (no `Context` on ViewModel methods — your new repository methods are subject to this), and Lesson 008 (JVM test patterns: `TemporaryFolder`, single `TestDispatcher`) are especially relevant.
...
- **`MainActivity.kt`** — add `Trim` and `Processing` branches to the conditional routing.
